...
parser.add_argument('-to', '--test_organ', type=str, default='../data/AMOS2022_ct_pancreas')
...
class NIIDataset_Union_ALL(Dataset): 
    ...
    def _set_file_paths(self, paths):
        ...
        for sub_dataset in os.listdir(paths):
            cur_sub_datapath = os.path.join(paths, sub_dataset)
            # for path in cur_sub_datapath:
            for dt in ['Ts', 'Val']:
                d = os.path.join(cur_sub_datapath, f'labels{dt}')
                if os.path.exists(d):
                    for name in os.listdir(d):
                        base = os.path.basename(name).split('.nii.gz')[0]
                        self.image_paths.append(f"{cur_sub_datapath}/images{dt}/{base}.nii.gz")
                        self.label_paths.append(f"{cur_sub_datapath}/labels{dt}/{base}.nii.gz")
