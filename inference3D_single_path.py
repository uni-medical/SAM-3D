...
parser.add_argument('-dt', '--data_type', type=str, default='RVa')
...
def finetune_model_predict3D(img3D, gt3D, sam_trans, sam_model_tune, device='cuda:0', click_method='random',num_clicks=5, prev_masks=None):
    # H, W = img_np.shape[:2]
    H, W, D = img3D.shape[:3]
...
for image3D, gt3D in tqdm(train_dataloader):
    ...
    seg_mask_list, points, labels, iou_list, dice_list = finetune_model_predict3D(image3D, gt3D, sam_trans, sam_model_tune, device=device, click_method=args.point_method, num_clicks=args.num_clicks, prev_masks=None)

    
    # print(labels)

    for point, label in zip(points, labels):
        if gt3D[0][0][point[0][0]][point[0][1]][point[0][2]].item() != label[0]:
            print('wrong')
...
jls_extract_var('mean IoU : ', sum(all_iou_list)/len(all_iou_list))
